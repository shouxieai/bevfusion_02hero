...
def perpare_val_dataset(cocodir, batch_size=8):
...
import yaml
def perpare_train_dataset(cocodir, batch_size=8):
    # 训练集需要增广
    # 加载增广配置文件
    ...
    for name, module in model.named_modules():
        if isinstance(module, quant_nn.TensorQuantizer): #遍历所有信息。直到层里有quant_nn.TensorQuantizer
            if module._calibrator is not None: #如果有校准器
                ...
                module.disable() #直接禁用该模块，即禁用这个量化层
    
    # 模型前向收集信息 test
    with torch.no_grad():
        for i, datas in tqdm(enumerate(data_loader), total=num_batch):
            imgs = datas[0].to(device, non_blocking=True).float() / 255.0 # 图片从uint8转float 再归一化
            # datas是一个列表。第一个元素是(4, 3, 640, 640)的照片。 
            # 第二个元素是边框（39， 6）
            # 第三个元素是长度为4的列表真实图片路径
            # 第四个元素是长度为4的列表里面全是None
            ...
            t = time.time()
            with torch.no_grad():
                model(imgs) # 第一个batch要6秒，后面所有的都要30秒左右。可能跟使用train_dataloader有关
                print(f"{time.time() - t}")
                if i >= num_batch:
                    break
    ...
    for name, module in model.named_modules():
        if isinstance(module, quant_nn.TensorQuantizer):
            if module._calibrator is not None:
                ...
                module._amax = module._amax.to(device) # 注意：计算结束后，把值放到device上
...
def calibrate_model(model, dataloader, device):
    ## 8.1.1 收集信息
    collect_stats(model, dataloader, device, 200) # 调用8.1.1.1中定义的函数
...
def have_quantizer(layer):
    for name, module in layer.named_modules():
        if isinstance(module, quant_nn.TensorQuantizer):
            return True
# 10.2 
class disable_quantization: # 类似于上下文管理器的工作。作用：临时禁制模型该层的量化
    # 初始化
